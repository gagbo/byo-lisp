...
static mpc_val_t *
mpc_parse_apply_to(mpc_input_t *i, mpc_apply_to_t f, mpc_val_t *x,
                   mpc_val_t *d) {
...
static int
mpc_parse_run(mpc_input_t *i, mpc_parser_t *p, mpc_result_t *r, mpc_err_t **e) {
    int j = 0, k = 0;
        ...
        case MPC_TYPE_MANY:
            ...
            while (mpc_parse_run(i, p->data.repeat.x, &results[j], e)) {
                ...
                if (j == MPC_PARSE_STACK_MIN) {
                    results_slots = j + j / 2;
                    results =
                        mpc_malloc(i, sizeof(mpc_result_t) * results_slots);
...
int
mpc_parse(const char *filename, const char *string, mpc_parser_t *p,
          mpc_result_t *r) {
    int x;
...
int
mpc_nparse(const char *filename, const char *string, size_t length,
...
int
mpc_parse_file(const char *filename, FILE *file, mpc_parser_t *p,
               mpc_result_t *r) {
    int x;
...
int
mpc_parse_pipe(const char *filename, FILE *pipe, mpc_parser_t *p,
               mpc_result_t *r) {
    int x;
...
mpc_parser_t *
mpc_check(mpc_parser_t *a, mpc_check_t f, const char *e) {
    mpc_parser_t *p = mpc_undefined();
...
mpc_parser_t *
mpc_check_with(mpc_parser_t *a, mpc_check_with_t f, void *x, const char *e) {
    mpc_parser_t *p = mpc_undefined();
...
mpc_parser_t *
mpc_checkf(mpc_parser_t *a, mpc_check_t f, const char *fmt, ...) {
    va_list va;
    char *buffer;
    mpc_parser_t *p;
...
mpc_parser_t *
mpc_check_withf(mpc_parser_t *a, mpc_check_with_t f, void *x, const char *fmt,
                ...) {
    va_list va;
    char *buffer;
    mpc_parser_t *p;
...
static int
mpc_soi_anchor(char prev, char next) {
    (void)next;
    return (prev == '\0');
}
static int
mpc_eoi_anchor(char prev, char next) {
    (void)prev;
    return (next == '\0');
}
...
int
mpc_test_fail(mpc_parser_t *p, const char *s, const void *d,
              int (*tester)(const void *, const void *), mpc_dtor_t destructor,
...
int
mpc_test_pass(mpc_parser_t *p, const char *s, const void *d,
              int (*tester)(const void *, const void *), mpc_dtor_t destructor,
              void (*printer)(const void *)) {
    mpc_result_t r;
    if (mpc_parse("<test>", s, p, &r)) {
...
static mpc_val_t *
mpcaf_grammar_regex(mpc_val_t *x, void *s) {
    mpca_grammar_st_t *st = s;
